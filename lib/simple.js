	...
	const sock = Object.defineProperties(conn, {
		...
		profilePictureUrl: {
			async value(jid, type = 'image', query = 'url') {
				try {
					...
					if (query == 'buffer') {
						avatarCache = avatarCache || fs.readFileSync(path.resolve(__dirname, '../media/avatar_contact.png'));
						return avatarCache;
					}
		...
		getLid: {
			value(jid) {
				...
				const reversed = conn._lidReverse.get(jid);
		...
		sendFile: {
			...
			async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
				...
				if (fileSize >= 100) throw 'File size is too big!';
				const opt = {};
		...
		sendSticker: {
			async value(jid, path, quoted, options = {}) {
				...
				if (data.length === 0) throw 'File tidak ditemukan';
		...
		sendMedia: {
			 ...
			 */
			async value(jid, path, quoted, options = {}) {
				const { mime, data } = await conn.getFile(path);
		...
		sendAlbum: {
			async value(jid, medias = [], options = {}) {
				if (medias.length < 2) throw 'Album minimal berisi 2 media.';
		...
		sendAlbumMessage: {
			async value(jid, medias, options = {}) {
				...
				if (!Array.isArray(medias) || medias.length < 2) throw 'Album minimal berisi 2 media.';
...
END:VCARD
		...
		loadMessage: {
			 ...
			 */
			value(messageID) {
				for (const [, chat] of Object.entries(conn.chats)) {
					if (typeof chat?.messages !== 'object') continue;
					const msg = chat.messages[messageID];
					if (msg) return msg;
					for (const [, v] of Object.entries(chat.messages)) {
						if (v.key?.id === messageID) return v;
					}
				}
			},
			enumerable: true,
...
export function serialize() {
	...
	return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		chat: {
			get() {
				return this.conn?.getJid(isPnUser(this.key?.remoteJidAlt) ? this.key.remoteJidAlt : this.key?.remoteJid);
			},
		},
		isGroup: {
			get() {
